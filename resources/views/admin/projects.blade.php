                ...
                <tbody>
                    @foreach($projects as $project)
                        <tr>
                            ...
                            <td>{{ $project->code }}</td>
                            <td>{{ $project->lead?->user_name ?? '-' }}</td>
                            <td>{{ $project->members->pluck('user_name')->join(', ') ?: '-' }}</td>
                            <td><div class="flex flex-wrap gap-2"><a href="{{ route('admin.projects.devices', $project) }}" class="btn-soft">Devices</a><button type="button" class="btn-soft" data-open-dialog="project-edit-{{ $project->id }}">Edit</button><button type="button" class="btn-soft" data-open-dialog="project-delete-{{ $project->id }}">Delete</button></div></td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
...
<dialog id="project-create-dialog" class="max-w-4xl"><form method="POST" action="{{ route('admin.projects.store') }}" data-ajax-form class="panel m-0">@csrf<div class="mb-4 flex items-start justify-between gap-3 border-b border-slate-200 pb-4"><h3 class="text-xl font-black">Create project</h3><button type="button" class="btn-soft" data-close-dialog>Close</button></div><div class="grid gap-4 md:grid-cols-2"><input class="field" name="name" placeholder="Project name" required><input class="field" name="code" placeholder="PROJECT-CODE"><textarea class="field md:col-span-2" name="description" placeholder="Description"></textarea><select class="field" name="lead_user_id"><option value="" disabled hidden>Select coordinator</option>@foreach($coordinators as $coordinator)<option value="{{ $coordinator->id }}">{{ $coordinator->user_name }}</option>@endforeach</select><div class="md:col-span-2"><label class="label">Project members</label><div class="choice-panel"><div class="choice-list">@foreach($projectUsers as $member)<label class="choice-card"><input class="sr-only" type="checkbox" name="member_ids[]" value="{{ $member->id }}"><span class="choice-card-box"><span class="choice-indicator">✓</span><span><span class="block font-semibold text-slate-900">{{ $member->user_name }}</span><span class="mt-1 block text-xs text-slate-500">{{ \Illuminate\Support\Str::headline($member->user_role) }}</span></span></span></label>@endforeach</div></div><p class="mt-2 text-xs text-slate-500">Select users who will be members of this project.</p></div></div><div class="mt-5 flex justify-end"><button class="btn-primary" type="submit">Save project</button></div></form></dialog>

@foreach($projects as $project)
    <dialog id="project-edit-{{ $project->id }}" class="max-w-4xl"><form method="POST" action="{{ route('admin.projects.update', $project) }}" data-ajax-form class="panel m-0">@csrf @method('PATCH')<div class="mb-4 flex items-start justify-between gap-3 border-b border-slate-200 pb-4"><h3 class="text-xl font-black">Edit project</h3><button type="button" class="btn-soft" data-close-dialog>Close</button></div><div class="grid gap-4 md:grid-cols-2"><input class="field" name="name" value="{{ $project->name }}" required><input class="field" name="code" value="{{ $project->code }}"><textarea class="field md:col-span-2" name="description" placeholder="Description">{{ $project->description }}</textarea><select class="field" name="lead_user_id"><option value="" disabled hidden>Select coordinator</option>@foreach($coordinators as $coordinator)<option value="{{ $coordinator->id }}" @selected($project->lead_user_id === $coordinator->id)>{{ $coordinator->user_name }}</option>@endforeach</select><div class="md:col-span-2"><label class="label">Project members</label><div class="choice-panel"><div class="choice-list">@foreach($projectUsers as $member)<label class="choice-card"><input class="sr-only" type="checkbox" name="member_ids[]" value="{{ $member->id }}" @checked($project->members->contains('id', $member->id))><span class="choice-card-box"><span class="choice-indicator">✓</span><span><span class="block font-semibold text-slate-900">{{ $member->user_name }}</span><span class="mt-1 block text-xs text-slate-500">{{ \Illuminate\Support\Str::headline($member->user_role) }}</span></span></span></label>@endforeach</div></div><p class="mt-2 text-xs text-slate-500">Select users who will be members of this project.</p></div></div><div class="mt-5 flex justify-end"><button class="btn-primary" type="submit">Update project</button></div></form></dialog>
    <dialog id="project-delete-{{ $project->id }}" class="max-w-lg"><form method="POST" action="{{ route('admin.projects.destroy', $project) }}" data-ajax-form class="panel m-0">@csrf @method('DELETE')<div class="mb-4 flex items-start justify-between gap-3 border-b border-slate-200 pb-4"><h3 class="text-xl font-black">Delete project</h3><button type="button" class="btn-soft" data-close-dialog>Close</button></div><p class="text-sm text-slate-500">This project will be removed from the admin list and detached from related categories. Existing tickets will remain intact.</p><div class="mt-6 flex justify-end gap-2"><button type="button" class="btn-soft" data-close-dialog>Cancel</button><button class="btn-primary" type="submit">Delete project</button></div></form></dialog>
@endforeach
@endsection
