<div class="bg-white border border-slate-200 border-l-4 border-l-violet-500 rounded-xl overflow-hidden">
    <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100">
        <div class="flex items-center gap-3">
            <h3 class="text-sm font-bold text-slate-800">Technology (TECH)</h3>
            <span class="text-xs text-slate-400">{{ $rows->count() }} project</span>
        </div>
        @if($isAdmin)
            <button type="button" class="text-xs text-blue-600 hover:text-blue-800 font-semibold" onclick="openAddModal('project'); document.getElementById('fp-type').value='TECH'; syncDocPanel();">+ Add</button>
        @endif
    </div>
        ...
        <table class="min-w-full text-xs">
            <thead class="text-left text-slate-400 border-b border-slate-100 bg-slate-50/60">
                <tr>
                    ...
                    <th class="py-2 pr-3 font-medium">Opex Absorption</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($rows as $i => $p)
                    ...
                    <tr class="{{ $p->trashed() ? 'opacity-50 bg-slate-50' : 'hover:bg-slate-50/50' }}">
                        ...
                        <td class="py-1.5 pr-3 max-w-[150px] text-slate-600">{{ $p->mitra ?: '-' }}</td>
                        ...
                        <td class="py-1.5 pr-3 font-semibold text-blue-600 whitespace-nowrap">{{ $p->serapan ? $rp($p->serapan) : '-' }}</td>
                        <td class="py-1.5 pr-3 whitespace-nowrap">
                ...
                @empty
                    <tr><td colspan="13" class="px-4 py-8 text-center text-slate-400">No technology data.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
